...
//    	 echo "<div class='welcome'>Welcome:  ".$_COOKIE['user']."</div>";
require_once("db.php");
?>
<!doctype html>
<html>
...
<table class="tbl-qa">
  <thead>
	<tr>
	  <th class="table-header" width="5%"><div style="text-align:right;"><a href="add_r.php" class="button_link"><img src="crud-icon/add.png" title="Add New Record" style="vertical-align:bottom;" />Add</a></div ></th>
      <th class="table-header" width="5%">Nr.</th>
      <th class="table-header" width="5%">Retoure</th>
      <th class="table-header" width="10%">Verkaufer</th>
	  <th class="table-header" width="15%">Name</th>
	  <th class="table-header" width="5%">Typ</th>        
      <th class="table-header" width="8%">Datum</th>
      <th class="table-header" width="20%">Beschreibung</th>
	  <th class="table-header" width="5%">Rabatt</th>          
	  <th class="table-header" width="5%">Active</th>  
  ...
  <tbody id="table-body">
	...
	?>
	  <tr class="table-row">
        ...
        <td><?php echo $row["number_Invoice"]; ?></td>  
        <td><?php echo $row["retoure"]; ?></td>  
		<td><?php echo $row["name_seller"]; ?></td>          
        <td><?php echo $row["name_user"]; ?></td>
        <td><?php echo $row["sales"]; ?></td>          
        <td><?php echo date_format(date_create($row["date_Invoice"]),"d.m.Y"); ?></td>         
        <td><?php echo $row["statement_invoice"]; ?></td>
        <td><?php echo $row["rabatt_r"]; ?></td>          
          
        <td><?php echo '<a href="index.php?toggle=yes&id_Invoice='.$row["id_Invoice"].'&value='.$row["active"].'">'.$row["active"].'</a>'; ?></td> 
	    <td>
        <a href='edit_r.php?id_Invoice=<?php echo $row['id_Invoice']; ?>'><img src="crud-icon/edit.png" title="Edit" /></a></td>
              <td><a href="report.php?id_Invoice=<?php echo $row['id_Invoice']; ?>" target="_blank"><img src="crud-icon/is4.png" title="Print" /></a></td>
<!--          <td>-->
<!--		<a href='delete.php?id_Invoice=<?php echo $row['id_Invoice']; ?>'><img src="crud-icon/delete.png" title="Delete" /></a>-->
<!--		</td>-->
        <td> 
		<a href='rechnungen_detail.php?id_Invoice=<?php echo $row['id_Invoice']; ?>'>Rechnung Detail</a>
		</td>
	  </tr>
    <?php
      }
