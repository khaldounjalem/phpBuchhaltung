...
$sql22 = "INSERT INTO daily ( id_seller,id, art, date_daily, id_user, debit, credit, statement, number_user, name_user )
SELECT day.id_seller,day.id_day, day.art, day.date_day, day_detail.id_user, day_detail.debit, day_detail.credit, day_detail.statement_day_detail, users.number_user, users.name_user
...
//------------- Apend Rechnung kunde ---------
  $sql33 = "INSERT INTO daily ( id_seller,retoure, id, art, number_user, id_user, sales, name_user, statement, date_daily, debit )
SELECT invoice.id_seller,invoice.retoure, invoice.id_Invoice, invoice.art, users.number_user, users.id_user, invoice.sales, users.name_user, artikel.name_artikel, invoice.date_Invoice, quantity*price AS Expr1
...
//------------- Apend Rechnung Lieferant ---------
    $sql44 = "INSERT INTO daily ( id_seller,retoure, id, art, number_user, id_user, sales, name_user, statement, date_daily, credit )
SELECT invoice.id_seller,invoice.retoure, invoice.id_Invoice, invoice.art, users.number_user, users.id_user, invoice.sales, users.name_user, artikel.name_artikel, invoice.date_Invoice, quantity*price AS Expr1
FROM (invoice LEFT JOIN users ON invoice.id_user = users.id_user) LEFT JOIN (invoice_details LEFT JOIN artikel ON invoice_details.statement_Invoice_details = artikel.id_artikel) ON invoice.id_Invoice = invoice_details.id_Invoice
...
//---------------  Reture Kunde ----------------
    $sql55 = "INSERT INTO daily ( id_seller,retoure, id, art, number_user, id_user, sales, name_user, statement, date_daily, credit )
SELECT invoice.id_seller,invoice.retoure, invoice.id_Invoice, invoice.art, users.number_user, users.id_user, invoice.sales, users.name_user, artikel.name_artikel, invoice.date_Invoice, quantity*price AS Expr1
FROM (invoice LEFT JOIN users ON invoice.id_user = users.id_user) LEFT JOIN (invoice_details LEFT JOIN artikel ON invoice_details.statement_Invoice_details = artikel.id_artikel) ON invoice.id_Invoice = invoice_details.id_Invoice
...
//--------------- Reture Lieferant --------------------
    $sql66 = "INSERT INTO daily ( id_seller,retoure, id, art, number_user, id_user, sales, name_user, statement, date_daily, debit )
SELECT invoice.id_seller,invoice.retoure, invoice.id_Invoice, invoice.art, users.number_user, users.id_user, invoice.sales, users.name_user, artikel.name_artikel, invoice.date_Invoice, quantity*price AS Expr1
FROM (invoice LEFT JOIN users ON invoice.id_user = users.id_user) LEFT JOIN (invoice_details LEFT JOIN artikel ON invoice_details.statement_Invoice_details = artikel.id_artikel) ON invoice.id_Invoice = invoice_details.id_Invoice
WHERE (((invoice.retoure)='Yes') AND ((invoice.sales)='einkauf') AND ((invoice.active)=1))";
    ...
    $sql = "SELECT daily.id_seller, daily.id, daily.number_user, daily.id_user, daily.sales, daily.name_user, daily.debit, daily.credit, daily.statement, daily.date_daily, daily.art, daily.retoure, seller.name_seller
FROM daily LEFT JOIN seller ON daily.id_seller = seller.id_seller
WHERE ((sales LIKE '%$search_keyword%' or id_user LIKE '%$search_keyword%'or name_user LIKE '%$search_keyword%' or daily.retoure LIKE '%$search_keyword%') AND (daily.date_daily Between '$search2%' AND '$search3%' ) AND (daily.id_seller=$id_seller))
...
//  ---------------------  Total ------
    ...
    WHERE ((id_user LIKE '%$search_keyword%'or name_user LIKE '%$search_keyword%' or daily.retoure LIKE '%$search_keyword%') AND (daily.date_daily Between '$search2%' AND '$search3%' ) AND (daily.id_seller=$id_seller))";
...
<?php    
    ...
    <tr class="table-row">
    <td>ID Mitarbeiter:
    <?php echo $data4["id_seller"]; ?></td>
    <td>Nmae Mitarbeiter :
    <?php echo $data4["name_seller"]; ?></td>
    </tr>
    </table> 
<table class="tbl-qa">
  <thead>
